      ...
      >
        ...
        <el-form-item label="上传图片">
          ...
          <el-upload
              ...
              :before-upload="
              (file) => beforeUploadBase64(file, addUserForm.name)
            "
          >
...
const addUserRules = {
  ...
  file: [{ required: false, message: "请上传图片文件", trigger: "change" }],
};

const beforeUploadBase64 = (file: File, name: string) => {
  console.log("Before upload base64:", {
    name,
    fileName: file.name,
    addUserFormName: addUserForm.value.name,
  });
...
const handleUploadBase64 = async (base64String: string, name: string,file: File) => {
  ...
  try {
    ...
    const response = await my_axios.post<{
      ...
      message?: string;
      code?: number;
    }>(`/api/user/${encodeURIComponent(name)}/photo/upload`, {
      imageBase64: base64String,
      name: name,
    });
...
const handleAddUser = async () => {
  ...
  try {
    ...
    // });
    const response = await my_axios.post<User>("/api/user/add", postData);
...
const handleCustomUpload = async (options: any) => {
  ...
  // 确保 currentUser.value 被正确设置
  currentUser.value = users.value.find(user => user.name === user_id);
