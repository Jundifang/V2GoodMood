...
const handleCustomUpload = async (options: any) => {
  ...
  // 发送 JSON 请求
  try {
    const response = await my_axios.post(action, {
      image_base64: base64,
      user_id: user_id
    }, {
      headers: { 'Content-Type': 'application/json' }
...
const handleAddUser = async () => {
  ...
  try {
    // 1. 创建用户
    await my_axios.post('/api/user/add', {
      name: addUserForm.value.name
    });

    // 2. 如果有图片则上传
    if (addUserFile.value) {
      const base64 = await new Promise((resolve) => {
        const reader = new FileReader();
        reader.readAsDataURL(addUserFile.value!);
        reader.onload = () => resolve(reader.result?.toString().split(',')[1]);
      });

      await my_axios.post(
          `/api/user/${addUserForm.value.name}/photo/upload`,
          { image_base64: base64, user_id: addUserForm.value.name },
          { headers: { 'Content-Type': 'application/json' } }
      );
    }
